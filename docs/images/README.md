...
These are browser captures of the implemented Reframe interface, using the bundled Space Jam 1996 reference study. They are not generated UI mockups.
...
Screenshots contain application content only, without browser chrome, account settings, or credentials. They are static images; the comparison slider and other controls operate in the application, not inside GitHub's README.

Reframe's original interface is covered by the repository license. Space Jam content and artwork visible inside it retain their original ownership and are excluded from that grant. See [THIRD_PARTY_NOTICES.md](../../THIRD_PARTY_NOTICES.md).
